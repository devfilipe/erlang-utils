...
cd $(dirname $0)
code_path="'$(realpath `pwd`/../../_build/default/lib/{{name}}/ebin)'"
ret=""

# add service
{
curl -X POST -d ''${code_path}'' http://localhost:6464/cloudi/api/rpc/code_path_add.erl
ret=$(curl -X POST -d @{{name}}.conf http://localhost:6464/cloudi/api/rpc/services_add.erl)
} &> /dev/null

# check service
curl http://localhost:6464/debug/erlang/{{name}}
